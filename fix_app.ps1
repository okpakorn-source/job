...
window.submitApp=async function(e){
  ...
  try{
    ...
    console.log('[ATS] INSERT response:',applicant,'error:',insErr);
    if(insErr){alert(JSON.stringify(insErr));throw new Error('DB: '+insErr.message);}
    if(!applicant?.id) throw new Error('Insert ไม่สำเร็จ — ตรวจสอบ RLS policy');
    if(file){
...
$result = $top + $mid.Split("`n") + $bot
Set-Content "app.js" -Value $result -Encoding UTF8
