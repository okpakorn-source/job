...
async function renderDetail(){
  ...
  document.getElementById('page-detail').innerHTML=`<div class="detail-hero"><div class="detail-container"><button class="back-btn" onclick="navigate('home')">${svgBack} กลับรายการงาน</button>${j.image_url?`<img src="${j.image_url}" style="width:100%;max-height:250px;object-fit:cover;border-radius:var(--r);margin-bottom:16px"/>`:''}<div class="detail-header"><div class="detail-logo">${initials(j.title)}</div><div><div class="detail-title">${j.title}</div><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px"><span class="badge badge-dept">${j.department||''}</span><span class="badge badge-type">${j.type}</span>${j.status==='open'?'<span class="badge badge-new">✅ เปิดรับสมัคร</span>':'<span class="badge badge-urgent">🔒 ปิดรับสมัคร</span>'}</div><div class="detail-meta"><span>📍 ${j.location}</span><span>${svgMoney} ${sal}/เดือน</span><span>📅 ${ago(j.posted_at)}</span></div></div></div></div></div><div class="detail-body"><div class="detail-grid"><div><div class="detail-section"><h3>เกี่ยวกับตำแหน่งงาน</h3><p>${j.description||''}</p></div>${j.detailed_desc?`<div class="detail-section"><h3>รายละเอียดเพิ่มเติม</h3><div style="white-space:pre-wrap">${j.detailed_desc}</div></div>`:''}${j.requirements?`<div class="detail-section"><h3>คุณสมบัติที่ต้องการ</h3><div style="white-space:pre-wrap">${j.requirements}</div></div>`:''}${j.benefits?`<div class="detail-section"><h3>🎁 สวัสดิการ</h3><div style="white-space:pre-wrap">${j.benefits}</div></div>`:''}<div class="detail-section"><h3>ทักษะที่เกี่ยวข้อง</h3><div class="skill-tags">${(j.skills||[]).map(s=>`<span class="skill-tag">${s}</span>`).join('')}</div></div></div><div><div class="info-card"><h3>ข้อมูลตำแหน่ง</h3><div class="info-row"><span class="label">แผนก</span><span class="value">${j.department||'-'}</span></div><div class="info-row"><span class="label">รูปแบบ</span><span class="value">${j.type}</span></div><div class="info-row"><span class="label">สถานที่</span><span class="value">${j.location}</span></div><div class="info-row"><span class="label">เงินเดือน</span><span class="value" style="color:var(--green)">${sal}</span></div>${j.status==='open'?`<button class="btn btn-primary btn-lg apply-cta" onclick="navigate('apply','${j.id}')">สมัครงานตำแหน่งนี้</button>`:`<button class="btn btn-secondary btn-lg apply-cta" disabled>ปิดรับสมัครแล้ว</button>`}</div></div></div></div>`;
}
