...
async function renderHome(){
  ...
  try{
    ...
    const open=_sbJobs.filter(j=>j.status==='open');
    const hn=document.getElementById('hn-open');if(hn)hn.textContent=open.length;
...
async function renderDetail(){
  ...
  if(!j&&selJob){
    try{
      const {data}=await sb.from('jobs').select('*').eq('id',selJob).single();
      if(data) j=data;
    }catch(e){}
  }
...
async function renderApply(){
  // Load real jobs from Supabase (UUIDs)
  let jobs=[];
  try{
    const {data,error}=await sb.from('jobs').select('id,title,department').eq('status','open').order('posted_at',{ascending:false});
...
Id) throw new Error('กรุณาเลือกตำแหน่งที่สนใจ');
    ...
    const { data: applicant, error: insErr } = await sb.from('applicants').insert([{
      job_id: safeJobId,
      full_name: fullName,
      email,
      phone,
    ...
    // ── Success ───────────────────────────────────────────
    window._resumeFile = null;
    document.getElementById('page-apply').innerHTML = `<div class="apply-container"><div class="success-screen"><div class="success-icon">✅</div><h2>ส่งใบสมัครสำเร็จ!</h2><p class="text-muted">เราได้รับใบสมัครของคุณแล้ว ทีมงานจะติดต่อกลับภายใน 3-5 วันทำการ</p><div style="margin-top:32px;display:flex;gap:12px;justify-content:center"><button class="btn btn-secondary" onclick="navigate('home')">ดูตำแหน่งอื่น</button><button class="btn btn-primary" onclick="navigate('apply')">สมัครอีกครั้ง</button></div></div></div>`;
...
// Seed data
if (!DB.jobs.length) { DB.jobs = [{ id: 1, title: 'Senior Frontend Developer', dept: 'Engineering', type: 'Full-time', location: 'กรุงเทพฯ', salary: '80,000 – 120,000', status: 'open', posted: '2026-05-10', summary: 'พัฒนา UI/UX ระดับ Enterprise ด้วย React และ TypeScript สำหรับผู้ใช้กว่า 2 ล้านคน', description: 'พัฒนา UI components ด้วย React + TypeScript\nร่วมออกแบบ architecture กับทีม Backend\nCode review และ mentoring ทีม Junior', requirements: 'ประสบการณ์ React อย่างน้อย 3 ปี\nมีความรู้ TypeScript และ REST API\nมี portfolio แสดงผลงาน', skills: ['React', 'TypeScript', 'GraphQL', 'Git'] }, { id: 2, title: 'Product Manager', dept: 'Product', type: 'Full-time', location: 'Hybrid', salary: '90,000 – 150,000', status: 'open', posted: '2026-05-12', summary: 'กำหนดทิศทางผลิตภัณฑ์ เขียน PRD และ roadmap รายไตรมาส', description: 'กำหนด Product Vision และ Strategy\nวิเคราะห์ข้อมูลผู้ใช้และตลาด\nเขียน PRD, User Stories, Acceptance Criteria', requirements: 'ประสบการณ์ PM อย่างน้อย 3 ปี\nเข้าใจ Agile / Scrum\nภาษาอังกฤษระดับ Business', skills: ['Agile', 'SQL', 'Figma', 'JIRA'] }, { id: 3, title: 'UX/UI Designer', dept: 'Design', type: 'Full-time', location: 'กรุงเทพฯ', salary: '60,000 – 90,000', status: 'open', posted: '2026-05-08', summary: 'ออกแบบประสบการณ์ผู้ใช้ที่สวยงามสำหรับ web และ mobile application', description: 'Research และ understand user needs\nสร้าง wireframe, prototype และ final UI\nดูแลและพัฒนา Design System', requirements: 'ประสบการณ์ UX/UI อย่างน้อย 2 ปี\nเชี่ยวชาญ Figma\nมี portfolio แสดง UX process', skills: ['Figma', 'Prototyping', 'User Research', 'Design System'] }, { id: 4, title: 'Data Scientist', dept: 'Data & AI', type: 'Full-time', location: 'Remote', salary: '100,000 – 160,000', status: 'open', posted: '2026-05-13', summary: 'สร้าง ML models และ data pipeline เพื่อขับเคลื่อนการตัดสินใจของธุรกิจ', description: 'สร้างและ deploy ML models สำหรับ prediction\nวิเคราะห์ข้อมูลขนาดใหญ่ด้วย Python และ SQL\nทำ A/B testing และวัดผลลัพธ์', requirements: 'ปริญญาโทสาขา Statistics หรือ CS\nประสบการณ์ Python, scikit-learn อย่างน้อย 2 ปี\nรู้จัก cloud platform AWS หรือ GCP', skills: ['Python', 'ML', 'SQL', 'PyTorch', 'AWS'] }] }
navigate('home');
